...
{
    // TODO: adjust [addr] check against [USERTOP]
    if (addr >= p->sz || addr + 4 > p->sz || addr > USERTOP)
    {
        return -1;
    }
    *ip = *(int *)(addr);
...
int argptr(int n, char **pp, int size)
{
    ...
    if (argint(n, &i) < 0)
        return -1;
    // TODO: adjust [(uint)i] check against [USERTOP] and [PGSIZE]
    // [(uint)i] tracks where we're pointing, and it shouldn't point
    // anywhere that's larger than a PGSIZE and anything greater than
    // USERTOP.
    if ((uint)i >= proc->sz || (uint)i + size > proc->sz || (uint)i < PGSIZE || (uint)i > USERTOP)
        return -1;
    *pp = (char *)i;
    return 0;
}
